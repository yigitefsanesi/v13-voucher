...
run: async (client, message, args, embed, prefix) => { 
    ...
    {
    message.react(red)
    message.lineReply(`\`${prefix}kayıt <@Carloza/ID> <Isim> <Yaş>\``).then(x=>x.delete({timeout:5000})) 
    return }
...
.setThumbnail(uye.user.displayAvatarURL({ dynamic: true, size: 2048 }))
 ...
 let msg = await message.channel.send({ buttons : [ button_1, button_2, button_3 ], embed: Carloza})
